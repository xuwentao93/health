<?php
    ...
    $selectURL=mysqli_query($con,"SELECT url from sick where custom ='{$custom}';") or die('插入病人表失败');
    $url =array();
    ...
    for($i=0;$i<count($url);$i++){
        $customURL = "custom/".$url[$i][0];
        $openSick[$i] = fopen($customURL,"r") or die("Unabel to open file!");
